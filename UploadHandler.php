...
//    protected $uploadyear;
    ...
    const IMAGETYPE_WEBP = 'image/webp';
    const IMAGETYPE_JFIF = 'image/jfif';
...
//        $bobo = $_REQUEST['year'][1];
        $this->options = array(
            ...
            'inline_file_types' => '/\.(gif|jpe?g|jfif|webp|png)$/i',
            ...
            'accept_file_types' => '/\.(gif|jpe?g|jfif|webp|png)$/i',
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        return $this->setoptions['upload_dir'].$this->get_user_path()
    ...
    protected function get_download_url($file_name, $version = null, $direct = false) {
        if (!$direct && $this->options['download_via_php']) {
            $url = $this->setoptions['script_url']
                .$this->get_query_separator($this->setoptions['script_url'])
        ...
        return $this->setoptions['upload_url'].$this->get_user_path()
            .$version_path.rawurlencode($file_name);
    }

    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->setoptions['script_url']
            .$this->get_query_separator($this->setoptions['script_url'])
            .$this->get_singular_param_name()
    ...
    protected function imagevalidate_image_file($uploaded_file, $file, $error, $index) {
    ...
    protected function imagevalidate($uploaded_file, $file, $error, $index, $content_range) {
        if ($error) {
        ...
        if (!$content_range && $this->has_image_file_extension($file->name)) {
            return $this->imagevalidate_image_file($uploaded_file, $file, $error, $index);
        }
        return true;
    }
    ...
    protected function fix_file_extension($file_path, $name, $size, $type, $error,
        ...
        if (strpos($name, '.') === false &&
            preg_match('/^image\/(gif|jpe?g|jfif|webp|png)/', $type, $matches)) {
            $name .= '.'.$matches[1];
        }
    ...
    protected function gd_create_scaled_image($file_name, $version, $options) {
        ...
        list($file_path, $new_file_path) =
            self::get_scaled_image_file_paths($file_name, $version);
        ...
        $src_img = self::gd_get_image_object(
            $file_path,
            $src_func,
            !empty($options['no_cache'])
        );
        $image_oriented = false;
        if (!empty($options['auto_orient']) && self::gd_orient_image(
                $file_path,
                $src_img
            )) {
            $image_oriented = true;
        ...
        self::gd_set_image_object($file_path, $new_img);
        return $success;
    }
    ...
    protected function has_image_file_extension($file_path) {
        return !!preg_match('/\.(gif|jpe?g|jfif|webp|png)$/i', $file_path);
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file->type = $type;
        if ($this->imagevalidate($uploaded_file, $file, $error, $index, $content_range)) {
            ...
            if ($file_size === $file->size) {
                ...
                if ($this->has_image_file_extension($file->name)) {
                    if ($content_range && !$this->imagevalidate_image_file($file_path, $file, $error, $index)) {
                        unlink($file_path);
                    } else {
    ...
    protected function handle_form_data($file, $index) {
        $file->post_rel = @$_REQUEST['post_rel'][$index];
        $file->print_logo = @$_REQUEST['print_logo'][$index];
    }
    ...
    public function head() {
        ...
        $this->header('Cache-Control: no-store, no-cache, must-reimagevalidate');
